...
rs.addPath = function (n) {
 ...
 let hdim = 5;
  displacements[nm] = Point.mk(0,-80).plus(Point.mk(Math.random()*2*hdim-hdim,Math.random()*2*hdim-hdim));
...
rs.updateState = function () {
  let {walkers,goTowards,pstate,numSteps} = this;
  let {cstate} = pstate;
  let {time} = cstate;
  debugger;
  let ln = walkers.length;
